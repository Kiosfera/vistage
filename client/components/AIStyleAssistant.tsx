...
const initialMessage: Message = {
  id: '1',
  content: 'Olá! Eu sou a Luna, sua assistente de estilo pessoal com IA! 💫 Baseando-me na sua análise facial e preferências, posso te ajudar a criar looks incríveis. O que você gostaria de saber hoje?',
  sender: 'ai',
  timestamp: new Date(),
  suggestions: [
    'Que cores ficam melhor em mim?',
    'Como posso criar um look para trabalho?',
    'Que acessórios combinam comigo?',
    'Dicas para um encontro especial'
  ]
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button
          ...
          onClick={() => handleSuggestionClick('Que cores ficam melhor em mim?')}
        >
          <Sparkles className="w-5 h-5 mb-2 text-primary" />
          <span className="font-medium text-sm">Minha Paleta</span>
          <span className="text-xs text-muted-foreground">Descubra suas cores ideais</span>
        </Button>
        
        <Button
          variant="outline"
          className="h-auto p-4 text-left flex-col items-start"
          onClick={() => handleSuggestionClick('Como posso criar um look para trabalho?')}
        >
          <Heart className="w-5 h-5 mb-2 text-primary" />
          <span className="font-medium text-sm">Look Profissional</span>
          <span className="text-xs text-muted-foreground">Dicas para o trabalho</span>
        </Button>
      </div>
    </div>
  );
